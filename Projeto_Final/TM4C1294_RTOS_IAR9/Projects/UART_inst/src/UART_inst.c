...
enum estados_elevador {PARADO,ESPERANDO,MOVIMENTO,SOLICITA_MOVIMENTO};
 ...
 char msg[16];
  ...
  while(1){
         ...
         status = osMessageQueueGet(*fila_m, &msg, NULL, 0U);   // wait for message
          if (status == osOK) {
            ...
            {   
                ...
                if(aux_numb==andar && estado_elevador==PARADO)
                  {
                    osMutexAcquire(uart_id, osWaitForever);
            ...
            else//Mensagem de mudanca de andar
            {
                ...
                else if((andares_pendentes[aux_numb]==2 && direcao==DESCENDO && count_desc==1) || (andares_pendentes[aux_numb]==3 && direcao==SUBINDO && count_sub==1) )
                {
                  andares_pendentes[aux_numb]=0;
                    if (direcao==DESCENDO)
                     count_desc--;
      ...
      if((count_desc>0 || count_sub>0 )&& estado_elevador==SOLICITA_MOVIMENTO)
      {
        ...
        else if((estado_elevador==ESPERANDO || estado_elevador==PARADO || estado_elevador==SOLICITA_MOVIMENTO)  && count_desc==0 && count_sub==0 )
        {
         direcao=PARADO_DIR;
         estado_elevador=PARADO;
